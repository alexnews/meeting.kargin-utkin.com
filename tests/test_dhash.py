...
def test_one_more_bullet_barely_moves_the_hash() -> None:
    ...
    If an animated build moved the hash a lot, each step would commit as its own
    keyframe. It does not: adding a line changes few pixels, so the whole build
    stays one keyframe and the last frame of it is the finished slide.
    """
    partial = dhash_from_gray(gray_slide("Q3 Roadmap", ROADMAP[:2]))
